...
- `deepagent-human-ref`: OpenRouter HTTP経由で人間結果参照つきの候補探索を行うadapter
...
- `DeepAgentPocAppRunner`: OpenRouter HTTP + Qwenで評価対象アプリを代替する実LLM runner
...
実際のAPI形状が異なる場合は `HttpEndpointMap` または `HttpPocAppRunner` を拡張します。実PoCアプリAPIが未接続の段階でも、`--runner deepagent` によりOpenRouter/Qwenを評価対象として探索ループを回せます。
